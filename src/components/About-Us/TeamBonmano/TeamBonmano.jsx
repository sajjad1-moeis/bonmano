      ...
      <div className='container mb-20 '>
         <CarouselEsy count={3} isDots={false}>
            {Item.map((item) => (
               <div key={useId()}>
                  <div
                     className={`h-max my-auto bg-white flex gap-y-8 ${
                        item.isReverse ? "flex-col-reverse" : "flex-col"
                     }`}
                  >
                     <img
                        src={`/img/About-Us/TeamBonmano/${item.img}`}
                     ...
                     <div className={`  rounded-xl shadow-lg p-5`}>
                        <div className='flex gap-3 items-center justify-end'>
                           ...
                           <div className='bg-successDark p-1.5 rounded-md text-white text-lg w-max'>
                              <img src={`/img/About-Us/TeamBonmano/${item.smalImg}`} className='size-6' alt='' />
                           </div>
                        </div>
                        <p className='my-3 leading-7'>
                           واحد تحقیق و توسعه ما با تجربه‌ی سال‌ها پژوهش، تمامی جزئیات را با دقت بررسی می‌کند تا اطمینان
                           حاصل کند که قهوه نهایی بی‌نظیر است.
                        </p>
                     </div>
                  </div>
               </div>
